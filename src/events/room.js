...
const Events = [
	{ /* The Nomad  --  Merchant */
		...
		isAvailable: function() {
			return store.state.currentTab === 0 && store.state.stores.fur > 0
		},
		scenes: {
			'start': {
				text: [
				...
				buttons: {
					...
					'buyCompass': {
						available: function() {
							return (store.state.stores.compass || 0) < 1;
						},
						text: _('buy compass'),
	...
	{ /* Noises Outside  --  gain wood/fur */
		title: _('Noises'),
		isAvailable: function() {
			return store.state.currentTab === 0 && store.state.stores.woods !== undefined
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Noises Inside  --  trade wood for better good */
		...
		scenes: {
			...
			teeth: {
				...
				onLoad: function() {
					let numWood = store.state.stores.wood
					numWood = Math.floor(numWood * 0.1);
					if(numWood === 0) numWood = 1;
					let numTeeth = Math.floor(numWood / 5);
					if(numTeeth === 0) numTeeth = 1;
					actions.addM('stores', {'wood': -numWood, 'teeth': numTeeth})
				},
				buttons: {
					'leave': {
						text: _('leave'),
			...
			cloth: {
				...
				onLoad: function() {
					...
					actions.addM('stores', {'wood': -numWood, 'cloth': numCloth})
				},
				buttons: {
					'leave': {
						text: _('leave'),
	...
	{ /* The Beggar  --  trade fur for better good */
		title: _('The Beggar'),
		isAvailable: function() {
			return store.state.currentTab === 0 && store.state.stores.fur !== 'undefined'
		},
		scenes: {
			start: {
				text: [
	...
	{/* The Shady Builder */
		...
		isAvailable: function() {
			return store.state.currentTab === 0
				&& (store.state.buildings.hut || 0) >= 5
				&& (store.state.buildings.hut || 0) < 20;
		},
		scenes: {
			'start':{
				text: [
			...
			'build': {
				...
				onLoad: function() {
					...
					if(n < 20){
						actions.changeValue('buildings.hut', 1)
					}
				},
				buttons: {
					'end': {
	...
	{ /* Mysterious Wanderer  --  wood gambling */
		...
		isAvailable: function() {
			return store.state.currentTab === 0 && store.state.stores.wood !== undefined;
		},
		scenes: {
			start: {
				text: [
			...
			'wood100': {
				...
				action: function(inputDelay) {
					...
					setTimeout(function() {
						actions.changeValue('stores.wood', 300)
						actions.toast(_('the mysterious wanderer returns, cart piled high with wood.'))
					}, delay);
				},
				onLoad: function() {
					if(Math.random() < 0.5) {
			...
			'wood500': {
				...
				action: function(inputDelay) {
					var delay = inputDelay || false;
					setTimeout(function() {
						actions.changeValue('stores.wood', 1500)
						actions.toast(_('the mysterious wanderer returns, cart piled high with wood.'));
					}, delay);
				},
				onLoad: function() {
		...
		scenes: {
			...
			'fur100': {
				...
				action: function(inputDelay) {
					var delay = inputDelay || false;
					setTimeout(function() {
						actions.changeValue('stores.fur', 300)
						actions.toast(_('the mysterious wanderer returns, cart piled high with furs.'));
					}, delay);
				},
				onLoad: function() {
					if(Math.random() < 0.5) {
			...
			'fur500': {
				...
				action: function(inputDelay) {
					var delay = inputDelay || false;
					setTimeout(function() {
						actions.changeValue('stores.fur', 1500)
						actions.toast(_('the mysterious wanderer returns, cart piled high with furs.'));
					}, delay);
				},
				onLoad: function() {
	...
	{ /* The Scout  --  Map Merchant */
		title: _('The Scout'),
		isAvailable: function() {
			return store.state.currentTab === 0 && store.state.features.location.world;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* The Sick Man */
		...
		isAvailable: function() {
			return store.state.currentTab === 0 && (store.state.stores.medicine || 0) > 0;
		},
		scenes: {
			'start': {
				text: [
			...
			'alloy': {
				...
				onLoad: function() {
					actions.changeValue('stores.alien alloy', 1)
				},
				buttons: {
					'bye': {
						text: _('say goodbye'),
			...
			'cells': {
				...
				onLoad: function() {
					actions.changeValue('stores.energy cell', 3)
				},
				buttons: {
					'bye': {
						text: _('say goodbye'),
			...
			'scales': {
				...
				onLoad: function() {
					actions.changeValue('stores.scales', 5)
				},
				buttons: {
					'bye': {
						text: _('say goodbye'),
