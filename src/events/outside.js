...
const Events = [
	...
	title: _('A Ruined Trap'),
		isAvailable: function() {
			return store.state.currentTab === 1 && (store.state.buildings.trap || 0) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				onLoad: function() {
					var numWrecked = Math.floor(Math.random() * (store.state.buildings.trap || 0)) + 1;
                    actions.changeValue('buildings.trap', -numWrecked)
				},
	...
	{ /* Hut fire */
		title: _('Fire'),
		isAvailable: function() {
			return store.state.currentTab === 1 && (store.state.buildings.hut || 0) > 0 && (store.state.population || 0) > 50;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Sickness */
		title: _('Sickness'),
		isAvailable: function() {
			return store.state.currentTab === 1 && (store.state.population || 0) > 10 && (store.state.population || 0) < 50 && (store.state.stores.medicine || 0) > 0;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Plague */
		title: _('Plague'),
		isAvailable: function() {
			return store.state.currentTab === 1 && (store.state.population || 0) > 50 && (store.state.stores.medicine || 0) > 0;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Beast attack */
		...
		isAvailable: function() {
			return store.state.currentTab === 1 && (store.state.population || 0) > 0;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Soldier attack */
		...
		isAvailable: function() {
			return store.state.currentTab === 1 && (store.state.population || 0) > 0 && store.state.cityCleared;
		},
		scenes: {
			'start': {
				text: [
